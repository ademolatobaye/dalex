(function ($) {
	...
	if($('.active-sticky-header').length){
		const $stickyHeader = $("header.main-header .header-sticky");
		...
		function setHeaderHeight(){
			$("header.main-header").css("height", $stickyHeader.outerHeight());
		}
		...
		$window.on("scroll", function() {
			...
			$stickyHeader.toggleClass("active", (fromTop > 24));
		});
	}
	...
	/* Color Theme Toggle */
	const storageKey = 'dalex-theme';

	function applyTheme(theme) {
		const resolvedTheme = theme === 'dark' ? 'dark' : 'light';
		document.body.setAttribute('data-theme', resolvedTheme);
		$('.theme-toggle').attr('aria-pressed', resolvedTheme === 'dark');
	}
	...
	/* Homepage Metric Counter */
	if ($('.metric-counter').length) {
		const animateMetricCounter = (counter) => {
